...
describe('Room', () => {
  ...
  describe('Persistence', () => {
    it('should save data', async () => {
      ...
      await room.save('progress', { level: 5, score: 1000 })
      
      expect(mockFetch).toHaveBeenCalledWith(
        expect.stringContaining('/v1/connect/saves/progress'),
        expect.objectContaining({
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ level: 5, score: 1000 })
        })
      )
    })
    ...
    it('should load data', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({ level: 5, score: 1000 })
      })
      
      const data = await room.load('progress')
      
      expect(data).toEqual({ level: 5, score: 1000 })
      expect(mockFetch).toHaveBeenCalledWith(
        expect.stringContaining('/v1/connect/saves/progress'),
    ...
    it('should return null for non-existent save', async () => {
      ...
      const data = await room.load('nonexistent')
    ...
    it('should throw on save error', async () => {
      ...
      await expect(room.save('test', {})).rejects.toThrow('Storage full')
    })
    ...
    it('should delete save', async () => {
      ...
      await room.deleteSave('progress')
    ...
    it('should list saves', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({ keys: ['progress', 'settings', 'achievements'] })
      })
      
      const keys = await room.listSaves()
      
      expect(keys).toEqual(['progress', 'settings', 'achievements'])
    })
    ...
    it('should URL-encode save keys', async () => {
      ...
      await room.save('level/boss#1', { defeated: true })
      
      expect(mockFetch).toHaveBeenCalledWith(
        expect.stringContaining(encodeURIComponent('level/boss#1')),
        expect.anything()
      )
    })
  })
